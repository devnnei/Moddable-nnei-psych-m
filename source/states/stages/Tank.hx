...
{
	var picoAnimLast:String = "";
	var picoAnimFinished:Bool = false;
	...
	override function update(elapsed:Float)
	{
		...
		// Poll for pico animation completion
		if (pico != null && pico.anim != null && pico.anim.curInstance != null && pico.animation != null && pico.animation.curAnim != null)
		{
			var curAnimName = pico.anim.curInstance.symbol.name;
			if (pico.animation.curAnim.finished && !picoAnimFinished)
			{
				picoAnimFinished = true;
				picoStressCycle();
			}
			if (!pico.animation.curAnim.finished)
			{
				picoAnimFinished = false;
			}
			picoAnimLast = curAnimName;
		}
	}
	...
	function picoStressCycle() {
		switch (pico.anim.curInstance.symbol.name) {
			...
			case "picoEnd", "Pico Dual Wield on Speaker idle":
				gfGroup.alpha = 1;
				pico.visible = false;
				// No need to remove callback, finishCallback will be replaced if needed
		}
	}
	...
	function stressIntro()
	{
		...
		// Initialize pico animation tracking
		picoAnimLast = "";
		picoAnimFinished = false;
