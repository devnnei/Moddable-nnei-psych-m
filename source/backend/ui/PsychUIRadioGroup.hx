...
class PsychUIRadioGroup extends FlxSpriteGroup
{
	...
	{
		// Only destroy if object implements IFlxDestroyable
		if (Std.isOfType(_hitbox, flixel.util.IFlxDestroyable))
			_hitbox = FlxDestroyUtil.destroy(_hitbox);
		if (Std.isOfType(arrowUp, flixel.util.IFlxDestroyable))
			arrowUp = FlxDestroyUtil.destroy(arrowUp);
		if (Std.isOfType(arrowDown, flixel.util.IFlxDestroyable))
